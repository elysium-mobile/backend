...
public class PerformanceAssembler {
    ...
    public static CreatePerformanceCommand toCommandFromRequest(CreatePerformanceRequest request) {
        return new CreatePerformanceCommand(new EmployeeProfileId(request.employeeProfileId()), request.dateTime(), request.classification());
    }
    ...
    public static PerformanceResponse toResponseFromEntity(Performance performance) {
        return new PerformanceResponse(performance.getId(), performance.getEmployeeProfileId().employeeProfileId(), performance.getDateTime(), performance.getClassification());
    }
}
