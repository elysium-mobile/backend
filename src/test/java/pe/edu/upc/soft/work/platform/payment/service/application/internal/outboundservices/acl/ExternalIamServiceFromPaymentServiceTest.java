...
class ExternalIamServiceFromPaymentServiceTest {

    @Mock
    private IamContextFacade iamContextFacade;

    @InjectMocks
    private ExternalIamServiceFromPaymentService service;
    ...
    @Test
    @DisplayName("existEmployeeProfileId(Long) -> returns true when facade confirms existence (AAA)")
    void existEmployeeProfileIdReturnsTrue() {
        // Arrange
        when(iamContextFacade.existsEmployeeProfileById(8L)).thenReturn(true);

        // Act
        boolean result = service.existEmployeeProfileId(8L);
    ...
    @Test
    @DisplayName("existEmployeeProfileId(Long) -> returns false when facade reports absence (AAA)")
    void existEmployeeProfileIdReturnsFalse() {
        // Arrange
