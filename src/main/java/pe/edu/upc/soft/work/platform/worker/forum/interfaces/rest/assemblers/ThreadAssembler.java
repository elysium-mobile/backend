...
public class ThreadAssembler {
    ...
    public static CreateThreadCommand toCommandFromRequest(CreateThreadRequest request) {
        return new CreateThreadCommand(request.title(), request.areaCompanyId(), request.lastMessage());
    }
    ...
    public static UpdateThreadCommand toCommandFromRequest(Long threadId, UpdateThreadRequest request) {
        return new UpdateThreadCommand(threadId, request.title(), request.areaCompanyId(), request.lastMessage());
    }
    ...
    public static ThreadResponse toResponseFromEntity(Thread thread) {
        return new ThreadResponse(thread.getId(), thread.getTitle(), thread.getAreaCompanyId(), thread.getLastMessage());
    }
}
