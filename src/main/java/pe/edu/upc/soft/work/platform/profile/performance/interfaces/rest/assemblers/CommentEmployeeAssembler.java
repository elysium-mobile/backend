...
public class CommentEmployeeAssembler {
    ...
    public static CreateCommentEmployeeCommand toCommandFromRequest(CreateCommentEmployeeRequest request) {
        return new CreateCommentEmployeeCommand(request.title(), request.content(),new RRHHProfileId( request.rrhhProfileId()), request.performanceId());
    }
    ...
    public static UpdateCommentEmployeeCommand toCommandFromRequest(Long commentemployeeId, UpdateCommentEmployeeRequest request) {
        return new UpdateCommentEmployeeCommand(commentemployeeId, request.title(), request.content(),new RRHHProfileId( request.rrhhProfileId()), request.performanceId());
    }
    ...
    public static CommentEmployeeResponse toResponseFromEntity(CommentEmployee commentemployee) {
        return new CommentEmployeeResponse(commentemployee.getId(), commentemployee.getTitle(), commentemployee.getContent(), commentemployee.getRrhhProfileId().rrhhProfileId(), commentemployee.getPerformanceId());
    }
}
